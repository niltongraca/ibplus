  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/quotes", {
        ...
        body: JSON.stringify({ customer, validUntil: validUntil || null, notes, items }),
      });
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-ib-primary">Novo Orçamento</h1>
          <p className="text-ib-muted text-sm">Criar orçamento para enviar ao cliente</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <tr key={i} className="border-b border-gray-50">
                    <td className="p-1">
                      <input
                        ...
                        className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ib-accent/40"
                        required
                      />
                    </td>
                    <td className="p-1">
                      <input
                        ...
                        className="w-16 px-2 py-2 border border-gray-200 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-ib-accent/40"
                        required
                      />
                    </td>
                    <td className="p-1">
                      <input
                        ...
                        className="w-full px-2.5 py-2 border border-gray-200 rounded-lg text-sm text-right focus:outline-none focus:ring-2 focus:ring-ib-accent/40"
                        required
                      />
                    </td>
                    <td className="p-1 text-right font-semibold text-ib-primary">
        ...
        <div className="flex justify-end gap-3">
          <Link
            href="/finance/orcamentos"
            className="px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-medium text-ib-muted hover:bg-gray-50 transition-colors"
          >
            Cancelar
          </Link>
          <button
            type="submit"
            disabled={saving}
            className="flex items-center gap-2 bg-ib-accent hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
            <Save className="w-4 h-4" /> {saving ? "A salvar..." : "Salvar Orçamento"}
          </button>
        </div>
      </form>
    </div>
