...
async function recordTransaction(companyId: string, input: TransactionInput, db: DbClient = prisma): Promise<void> {
...
export async function removeTransactionsByRef(companyId: string, refType: string, refId: string, db: DbClient = prisma): Promise<void> {
  await db.transaction.deleteMany({ where: { companyId, refType, refId } });
}
