...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white rounded-xl border border-gray-200 shadow-lg z-50 max-h-[70vh] flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            <h3 className="font-semibold text-ib-primary text-sm">Notificações</h3>
            {unreadCount > 0 && (
              ...
              notifications.slice(0, 20).map((n) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`text-sm ${!n.read ? "font-semibold text-ib-primary" : "text-ib-primary"}`}>{n.title}</p>
                    {n.message && <p className="text-xs text-ib-muted mt-0.5 line-clamp-2">{n.message}</p>}
                    <p className="text-[10px] text-ib-muted mt-1">{timeAgo(n.createdAt)}</p>
                  </div>
